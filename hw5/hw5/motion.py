...
def lucas_kanade(img1, img2, keypoints, window_size=5):
    ...
    for y, x in keypoints:
        ...
        # neighborhood window
        y_start, y_end = max(0, y - w), min(img1.shape[0], y + w + 1)
        x_start, x_end = max(0, x - w), min(img1.shape[1], x + w + 1)

        # getting gradient values
        window_Ix = Ix[y_start:y_end, x_start:x_end].flatten()
        window_Iy = Iy[y_start:y_end, x_start:x_end].flatten()
        window_It = It[y_start:y_end, x_start:x_end].flatten()

        # make the A matrix and b vector
        A = np.column_stack((window_Ix, window_Iy))
        b = -window_It
...
def iterative_lucas_kanade(img1, img2, keypoints,
    ...
    for y, x, gy, gx in np.hstack((keypoints, g)):
        ...
        # neighborhood window
        y_start, y_end = max(0, y1 - w), min(img1.shape[0], y1 + w + 1)
        x_start, x_end = max(0, x1 - w), min(img1.shape[1], x1 + w + 1)
        ...
        for k in range(num_iters):
            ...
            y2 = int(round(y+gy+vy)); x2 = int(round(x+gx+vx))
             ...
             # neighborhood window
            y2_start, y2_end = max(0, y2 - w), min(img2.shape[0], y2 + w + 1)
            x2_start, x2_end = max(0, x2 - w), min(img2.shape[1], x2 + w + 1)

            window_It = img1[y_start:y_end, x_start:x_end] - img2[y2_start:y2_end, x2_start:x2_end]
            window_It = window_It.flatten()

            b = np.array([np.sum(window_It * window_Ix), np.sum(window_It * window_Iy)])
            vk = G_inv @ b
...
def pyramid_lucas_kanade(img1, img2, keypoints,
    ...
    for L in range(level, -1, -1):
        pyramid_level = keypoints / (scale ** L)

        d = iterative_lucas_kanade(pyramid1[L], pyramid2[L], pyramid_level, 
...
def IoU(bbox1, bbox2):
    ...
    union_A = bbox1_A + bbox2_A - intersection

    score = intersection / union_A
